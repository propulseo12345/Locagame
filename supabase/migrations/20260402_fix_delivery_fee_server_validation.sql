-- ══════════════════════════════════════════════════════════════════════════════
-- Migration: Frais de livraison SERVER-AUTHORITATIVE (delivery_zones)
--
-- FAILLE CORRIGÉE: La migration 20260316 rendait p_delivery_fee client-authoritative.
-- Un utilisateur malveillant pouvait envoyer delivery_fee=0 pour obtenir la
-- livraison gratuite. Le frontend n'a plus aucun contrôle sur les frais.
--
-- SOLUTION: Le serveur recalcule les frais depuis la table delivery_zones.
--   • delivery → lookup zone_id → delivery_zones.delivery_fee
--   • Si sous-total ≥ free_delivery_threshold → frais = 0
--   • pickup → toujours 0
--   • Zone introuvable → log sécurité + frais = 0
--
-- CHANGEMENTS:
--   1. validate_and_create_reservation: supprime p_delivery_fee, calcule depuis zones
--   2. create_guest_checkout: supprime p_delivery_fee du call interne
-- ══════════════════════════════════════════════════════════════════════════════


-- ─────────────────────────────────────────────────────────────────────────────
-- 1. DROP l'ancienne signature (p_delivery_fee en paramètre 11)
-- ─────────────────────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS validate_and_create_reservation(
  jsonb, date, date, text, uuid, uuid, jsonb, numeric, boolean, boolean, numeric
);


-- ─────────────────────────────────────────────────────────────────────────────
-- 2. RECRÉER sans p_delivery_fee, avec calcul serveur depuis delivery_zones
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION validate_and_create_reservation(
  ...
  p_customer_id UUID,
  p_zone_id UUID DEFAULT NULL,          -- zone de livraison (lookup frais)
...
BEGIN
  ...
  FOR v_i IN 0..jsonb_array_length(p_items) - 1 LOOP
    ...
    -- 3b-bis. Bloquer les produits sans prix (tarif sur devis uniquement)
  ...
  -- ────────────────────────────────────────────────────
  -- 4. FRAIS DE LIVRAISON — SERVER-AUTHORITATIVE (delivery_zones)
  --    Le serveur calcule les frais depuis la table delivery_zones.
  --    Le client n'envoie PLUS de montant.
  -- ────────────────────────────────────────────────────
  IF p_delivery_type = 'pickup' THEN
    v_delivery_fee := 0;
  ELSE
    -- 4a. Résoudre la zone : p_zone_id direct ou fallback depuis l'adresse
    v_resolved_zone_id := p_zone_id;

    IF v_resolved_zone_id IS NULL AND (p_metadata->>'delivery_address_id') IS NOT NULL THEN
      SELECT a.zone_id INTO v_resolved_zone_id
      FROM addresses a
      WHERE a.id = (p_metadata->>'delivery_address_id')::UUID
        AND a.zone_id IS NOT NULL;
    END IF;

    -- 4b. Lookup frais depuis delivery_zones
    IF v_resolved_zone_id IS NOT NULL THEN
      SELECT dz.delivery_fee, dz.free_delivery_threshold
      INTO v_zone_fee, v_zone_threshold
      FROM delivery_zones dz
      WHERE dz.id = v_resolved_zone_id AND dz.is_active = true;

      IF NOT FOUND THEN
        RETURN jsonb_build_object('error', 'Zone de livraison invalide ou inactive');
      END IF;

      -- 4c. Livraison gratuite si sous-total ≥ seuil
      IF v_zone_threshold IS NOT NULL AND v_items_subtotal >= v_zone_threshold THEN
        v_delivery_fee := 0;
      ...
      END IF;
    ELSE
      -- 4d. Aucune zone trouvée → log sécurité + frais = 0
      v_delivery_fee := 0;
      INSERT INTO security_logs (user_id, event, details)
      VALUES (p_customer_id, 'delivery_without_zone',
        jsonb_build_object(
          'delivery_type',       p_delivery_type,
          'zone_id_param',       p_zone_id,
          'delivery_address_id', p_metadata->>'delivery_address_id',
          'note',                'Livraison sans zone identifiée – frais fixés à 0'
        ));
    END IF;
  END IF;
  ...
  --    Les frais de livraison sont désormais 100% serveur.
  -- ────────────────────────────────────────────────────
  ...
  v_pricing_breakdown := jsonb_build_object(
    ...
    'delivery_fee_source',   'server_zone_based',
    'zone_id',               v_resolved_zone_id,
    'free_delivery_applied', v_free_delivery,
    'deposit',               v_deposit,
    'total',                 v_total
  );

  -- ────────────────────────────────────────────────────
  -- 9. CRÉATION RÉSERVATION (prix 100% SERVEUR)
  -- ────────────────────────────────────────────────────
  INSERT INTO reservations (
    ...
    COALESCE(v_resolved_zone_id, p_zone_id),
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. METTRE À JOUR create_guest_checkout (supprimer p_delivery_fee du call)
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ────────────────────────────────────────────────────
  -- 2. CRÉER L'ADRESSE SI LIVRAISON (avec zone_id)
  -- ────────────────────────────────────────────────────
  ...
  --    p_delivery_fee SUPPRIMÉ — la RPC calcule depuis delivery_zones.
  -- ────────────────────────────────────────────────────
