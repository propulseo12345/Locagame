...
const menuItems: MenuItem[] = [
  ...
  { label: 'Interventions', href: '/technician/tasks', icon: Wrench },
  { label: 'Profil', href: '/technician/profile', icon: UserCircle },
];
...
export default function TechnicianLayout({ children }: TechnicianLayoutProps) {
  ...
  const userInitials = user ? `${user.firstName[0]}${user.lastName[0]}` : 'TC';
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      >
        <div className={`flex items-center border-b border-white/10 ${sidebarOpen ? 'px-5 py-4 justify-between' : 'px-0 py-4 justify-center flex-col gap-3'}`}>
          <Link to="/" className="flex items-center shrink-0 group">
            {sidebarOpen ? (
              ...
              <div className="w-10 h-10 rounded-xl bg-[#33ffcc] flex items-center justify-center text-[#000033] font-bold text-lg group-hover:scale-105 transition-transform">L</div>
            )}
          </Link>
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-white/60 hover:text-[#33ffcc] transition-colors" aria-label={sidebarOpen ? 'Réduire' : 'Ouvrir'}>
                ...
                <div key={item.href} className="relative group">
                  <Link
                    ...
                    className={`flex items-center rounded-xl transition-colors whitespace-nowrap ${
                      isActive ? 'bg-[#33ffcc] text-[#000033]' : 'text-white/70 hover:bg-white/10 hover:text-white'
                    } ${sidebarOpen ? 'h-11 px-4 gap-3' : 'h-11 w-11 justify-center mx-auto'}`}
                  >
        ...
        <div className={`border-t border-white/10 ${sidebarOpen ? 'p-4' : 'p-3'}`}>
          <div className={`flex items-center ${sidebarOpen ? '' : 'justify-center'}`}>
            <div className="w-10 h-10 rounded-full bg-[#33ffcc] flex items-center justify-center text-[#000033] font-bold text-sm shrink-0">{userInitials}</div>
            {sidebarOpen && (
      ...
      {/* Main content */}
      <div className="flex-1 overflow-auto min-w-0 pb-16 md:pb-0">
        {/* Top bar */}
          ...
          <div className="flex items-center justify-between px-4 py-3 md:px-8 md:py-4">
            <div className="flex items-center gap-3">
              ...
              <Link to="/" className="md:hidden">
                <img src="/logo-client.png" alt="LOCAGAME" className="h-6 w-auto" />
              </Link>
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <Link to="/" className="hidden md:block px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                ← Retour au site
              </Link>
              <button
                onClick={handleSignOut}
                className="flex items-center gap-2 px-3 py-2 text-sm bg-red-500/10 text-red-600 hover:bg-red-500/20 rounded-lg transition-all border border-red-500/30 active:scale-95"
              >
